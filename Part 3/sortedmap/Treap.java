...
public class Treap<K, V> extends AbstractMap<K, V> implements
	...
	public V put(K key, V value) {
		...
		TreapNode<K,V> e = new TreapNode<>(key, value, comparator);
		e.parent = parent;
	...
	/** From CLR */
	private void rotateLeft(TreapNode<K,V> p) {
		if (p != null) {
	...
	/** From CLR */
	private void rotateRight(TreapNode<K,V> p) {
		 if (p != null) {
    ...
    public V getValue(TreapNode<K, V> e) {
		 if (e == null) throw new NullPointerException();
		 return ((Entry<K,V>) e).getValue();
	}
	
	public K getKey(TreapNode<K, V> e) {
		 if (e == null) throw new NullPointerException();
		 return ((TreapNode<K,V>) e).getKey();
	}
	
	public int getPriority(TreapNode<K, V> e) {
		 if (e == null) throw new NullPointerException();
		 return ((TreapNode<K,V>) e).priority;
	}
	
	public Map.Entry<K, V> getLeft(TreapNode<K, V> e) {
		 if (e == null) throw new NullPointerException();
		 return ((TreapNode<K,V>) e).left;
	}
	
	public Map.Entry<K, V> getRight(TreapNode<K, V> e) {
		 if (e == null) throw new NullPointerException();
		 return ((TreapNode<K,V>) e).right;
	}
    ...
    class EntrySet extends AbstractSet<Map.Entry<K, V>> {
        ...
        public boolean remove(Object o) {
			...
			Map.Entry<K,V> entry = (Map.Entry<K,V>) o;
            V value = entry.getValue();
            TreapNode<K, V> p = getEntry(entry.getKey());
    ...
    final class SubMap<K, V> extends AbstractMap<K, V> implements
        ...
        class EntrySetView extends AbstractSet<Map.Entry<K, V>> {
            public Iterator<Map.Entry<K, V>> iterator() {
                return new Iterator<Map.Entry<K, V>>() {
                    ...
                    public void remove() { 
                    	...
                    	// deleted entries are replaced by their successors
                    	//if (lastReturned.left != null && lastReturned.right != null) next = lastReturned;
                    	Treap.this.remove(lastReturned.getKey());
                    	expectedModCount = modCount;
                    	lastReturned = null;
                    }
                };
			...
			public boolean remove(Object o) {
				...
				TreapNode<K,V> node = m.getEntry(key);
	 ...
	 * does not contain an entry for the key
	 */
	final TreapNode<K,V> getEntry(Object key) {
	 ...
	 * worthwhile here.)
	 */
	 final TreapNode<K,V> getEntryUsingComparator(Object key) {
		 @SuppressWarnings("unchecked")
		K k = (K)key;
	...
	private void deleteEntry(TreapNode<K,V> curr) {
		if (curr == null) return;
		if ( curr.left == null && curr.right == null){
			modCount++;
			size--;
			//System.out.println("Eventually:" + curr.key);
			if(curr.parent == null){
	...
	public void printTree ( Treap<K, V> t){
		printTree(t.root);
	}
	
	private void printTree( TreapNode <K,V> curr )
    {
        if( curr != null )
        {
        	 
            printTree( curr.left );
            System.out.println("Priority: " + String.valueOf(curr.priority) +
